...
export default function AuthModal({
  ...
  const googleClientId = process.env.NEXT_PUBLIC_GOOGLE_CLIENT_ID_NOVO || process.env.NEXT_PUBLIC_GOOGLE_CLIENT_ID;
  ...
  const handleGoogleCredentialResponse = async (response: any) => {
    ...
    try {
      ...
      setError('Não foi possível validar as credenciais do Google.');
    } finally {
  ...
  useEffect(() => {
    if (!isOpen || mode === 'forgot' || !googleClientId) return;
    let attempts = 0;
  ...
  const handleGoogleLogin = async () => {
    setError(null);
    if (!googleClientId) {
      setError('O login oficial com o Google requer um Google Client ID. Configure NEXT_PUBLIC_GOOGLE_CLIENT_ID nas variáveis de ambiente (.env.local ou Vercel).');
      return;
    }
